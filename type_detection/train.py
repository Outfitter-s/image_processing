...
from shared import TYPE_DETECTION_OUT_OUTPUT_H5_PATH, TYPE_DETECTION_OUT_CSV_PATH, TYPE_DETECTION_DIR, TYPE_DETECTION_OUT_IMAGES_DIR

if not os.path.exists(TYPE_DETECTION_DIR):
    raise FileNotFoundError(f"Dataset directory '{TYPE_DETECTION_DIR}' not found. Please download and extract the dataset using \"kaggle datasets download -d paramaggarwal/fashion-product-images-small\" and \"unzip fashion-product-images-small.zip\".")

def train_model_from_dataset():
  if os.path.exists(TYPE_DETECTION_OUT_OUTPUT_H5_PATH):
    return

  # 1. Load dataset metadata
  df = pd.read_csv(TYPE_DETECTION_OUT_CSV_PATH, on_bad_lines='skip', engine='python')
  ...
  train_gen = datagen.flow_from_dataframe(
      train_df,
      directory=TYPE_DETECTION_OUT_IMAGES_DIR,
      x_col='image',
      y_col='label',
  ...
  # 6. Train model
  callbacks = [
      ModelCheckpoint(TYPE_DETECTION_OUT_OUTPUT_H5_PATH, save_best_only=True, monitor="val_loss"),
  ...
  # 7. Save model
  model.save(TYPE_DETECTION_OUT_OUTPUT_H5_PATH)
